...
- **VLAJKA-VIEWER:** produkčná báza `https://peterferenc246-design.github.io/fia/viewer.html` (GitHub Pages, `.nojekyll`, žiadny build → commit = redeploy ~1 min; NIE jsDelivr — HTML tam nemá MIME na navigáciu, NIE githack — interstitial).
  Parametre: `doc`=DOC1 (klon prekladov `..._ALL.pdf`, jsDelivr @SHA), `doc2`=DOC2 (**podpísaný originál INTACT** na GitHube, jsDelivr @SHA — **nikdy nerozdeľuj**, PDF.js ho renderuje read-only a QES/PAdES zostáva platný), `orig`=**jsDelivr @SHA link na `doc2` v repe** (button „⬇ Podpísaný originál (QES)"; OneDrive/SharePoint sa NEPOUŽÍVA — nespúšťal stiahnutie. jsDelivr PDF vykreslí v prehliadači, `raw.githubusercontent.com` ho rovno stiahne — voľba podľa toho, čo Peter chce), `langs=CODE:PAGE[:2|:o],...` (`:2`=jazyk žije v doc2/podpísanom, `:o`=externý odkaz na `orig` v novom tabe — legacy), `d` (1|2 aktívny doc), `page` (úvodná strana).
  - **TLAČIDLO „✍ QES" — `qes=PAGE[:DOC[:ALIGN]]`, POVINNÉ vo všetkých 19 URL (od 19.07.2026).** Za poslednou vlajkou stojí zlaté tlačidlo, ktoré skočí priamo na **vizuálny podpisový blok** (fotka + časová pečiatka) v podpísanom origináli.
    **ZADÁVA SA VŽDY RUČNE — nikdy sa neodvodzuje.** Strana podpisu aj podpisový jazyk sa menia dokument od dokumentu (Peter vkladá QES pod ten jazyk, v ktorom podanie reálne odosiela). Bez parametra `qes=` sa tlačidlo NEVYKRESLÍ — to je zámer, nie chyba.
    **Hodnotu zisti PROGRAMOVO z podpísaného PDF, nehádaj** — podpisový widget nesie presné súradnice:
    ```python
    ...
    # ALIGN = ((H-y1)+(H-y0))/2 / H   ... stred bloku ako pomer výšky strany (H = mediabox.height)
    ```
    `DOC` = 1|2 (default 2, ak existuje `doc2`). `ALIGN`: `b` = spodok strany (default), `t` = vrch, `0..1` = pomer výšky strany — **preferuj pomer**, trafí podpis presne bez ohľadu na zoom a veľkosť okna.
    Príklad (podpis na str. 6 doc2, stred v 74,3 % výšky): `&qes=6:2:0.74`.
    **Zarovnanie skoku:** QES tlačidlo NESMIE skončiť na vrchu strany. `jumpTo(n, tries, align)` rieši `b` (spodok strany na spodok okna) aj pomer (vycentruje bod); pri prechode doc1→doc2 sa align nesie v URL ako `qj=`. Bežné vlajky skáču naďalej na vrch strany.
    **TLAČIDLÁ PRÁVNYCH PREDPISOV — `law=CELEX~POPIS[~TOOLTIP]` alebo `law=URL~POPIS[~TOOLTIP]`.** Parameter sa môže OPAKOVAŤ. Tlačidlá sedia vedľa „✍ QES", tmavomodré. Pri tvare `CELEX:` viewer poskladá EUR-Lex odkaz v jazyku AKTÍVNEJ vlajky (klik na SK vlajku → slovenské znenie), takže sa zadáva len CELEX kód, nie jazyková mutácia. `TOOLTIP` = plný názov predpisu po prejdení myšou; medzery píš ako `%20`.
    **Tooltip musí byť 9-jazyčný.** Text z URL je JEDNOJAZYČNÝ a slúži len ako fallback — preklady sú v mape `LAW_TITLES` vo `viewer.html`, kľúčované CELEX kódom, a prepínajú sa spolu s vlajkou. Pri každom NOVOM predpise doplň do `LAW_TITLES` všetkých 9 jazykov (DE/EN/SK/HR/PL/ES/IT/FR/SV) — inak sa vo všetkých jazykoch ukáže ten istý text z URL.
    **[AKTUALIZOVANÉ 22.07.2026 — pozri ★ ŠTANDARD nižšie: `law` je odteraz DEFAULT súčasť 3-tlačidlového setu pre odoslané podania; Claude vyberie nosný predpis daného dokumentu, a ak nie je zjavný, spýta sa Petra ktorý.]**
    **Kde je dnes nasadené (jediné miesto):** karta `case-dgcomp`, položka `cmt-dgcomp-05` (TFIA-2026-JV-009, odpoveď na odmietnutie prístupu k dokumentom) → `law=CELEX:32001R1049~1049/2001~Nariadenie%20Európskeho%20parlamentu%20a%20Rady%20(ES)%20č.%201049/2001%20z%2030.%20mája%202001%20o%20prístupe%20verejnosti%20k%20dokumentom`. Nikde inde v registri `law=` nie je a bez pokynu tam ani nepatrí.
    **TLAČIDLO „📄 Súhrn" — `sum=<ID modálu>`, napr. `&sum=summod-zaloba-03`.** Súhrn sa zobrazí **v okne PRIAMO VO VIEWERI** (čitateľ neopúšťa dokument), ale text sa **NEKOPÍRUJE**.
    **ŽELEZNÉ PRAVIDLO — JEDINÝ ZDROJ.** Text súhrnu existuje výhradne v `.summod` modáli na karte #303. Viewer si ho odtiaľ načíta cez verejné WP API (`public-api.wordpress.com/wp/v2/sites/foxprof.club/pages/303?_fields=content`), vyparsuje `#<ID modálu>`, rozdelí podľa `.gtl-b` a zobrazí podľa aktívnej vlajky. **Nikdy nezakladaj druhú kópiu textu** (JSON v repe, samostatná stránka a pod.) — pri zmene by sa rozišli. Rovnako to platí pre komentárové a Begleittext modály.
    Ak sa do `sum=` dá plná URL (`http…`), viewer ju otvorí ako obyčajný odkaz v novom okne. Pri zlyhaní načítania okno ponúkne odkaz do registra.
    **Cache viewera:** po každej zmene `viewer.html` bumpni `&vw=N` vo všetkých 19 URL — inak prehliadač drží starú verziu a zmena sa navonok neprejaví (aktuálne `vw=6`).
    **★ ŠTANDARD PRE ODOSLANÉ PODANIA — 3 TLAČIDLÁ, DEFAULT ZAPNUTÉ (od 22.07.2026):** za 9 vlajkami idú vždy tri tlačidlá v poradí **✍ QES · 📄 Súhrn · ⚖ predpis(y)**. Všetky tri patria do KAŽDEJ z 19 URL (`fallback` + 9× `urls` + 9× viditeľných `.open`). (Došlá pošta: QES ani law spravidla nie — riadi sa vetvou 4.1.)
      1. **✍ QES** — `&qes=STRANA[:DOC[:ALIGN]]`. `DOC=2` (podpísaný originál). `ALIGN`: 1-stranový list s podpisom naspodku → `:b` (napr. `&qes=1:2:b`); podpis v strede dlhšieho dokumentu → pomer výšky, napr. `&qes=6:2:0.74`. Stranu aj pomer vytiahni DETERMINISTICKY z podpisového poľa PDF, NIKDY neháduj ani nehádaj z textu:
        `PdfReader` → prejdi `page.get('/Annots')`, nájdi anotáciu s `/FT == '/Sig'` → index strany = STRANA, `Rect` → `ymid=(y0+y1)/2`.
        **POMER SA POČÍTA OD VRCHU: `ALIGN = 1 - ymid/H`** (H = `mediabox.height`). Overené na doc 12 DG COMP: podpis str. 6, `ymid/H=0.257` → `1-0.257=0.743` = bežiaci parameter `qes=6:2:0.74` ✓.
        Ak podpis sedí pri spodnom okraji strany (ymid/H < ~0.2), použi radšej `:b` — je robustnejšie než pomer.
        `SubFilter` (`/ETSI.CAdES.detached` alebo `/adbe.pkcs7.detached`) potvrdí, že ide o reálny QES/PAdES podpis. Ak `/Sig` pole CHÝBA, dokument nie je podpísaný → `qes=` NEPRIDÁVAJ.
      2. **📄 Súhrn** — `&sum=<ID summod modálu>` (napr. `&sum=summod-zaloba-p03`); ID = `summodId` v data-snap tej istej položky (jediný zdroj textu = modál na #303).
      3. **⚖ predpis** — `&law=<URL|CELEX>~POPIS~TOOLTIP`. Vyber NOSNÝ predpis daného podania: nemecký národný → `law=https://www.gesetze-im-internet.de/...~§ 66 GKG~<tooltip>` (napr. Erinnerung proti Kostenansatz → **§ 66 GKG**, `/gkg_2004/__66.html`); EÚ → `law=CELEX:...~popis~tooltip` (napr. prístup k dokumentom → 1049/2001). POPIS aj TOOLTIP url-encoduj (medzera `%20`, `§`=`%C2%A7`, pomlčka `—`=`%E2%80%94`). Ak nosný predpis nie je zjavný, spýtaj sa Petra KTORÝ.
      - **PORADIE tlačidiel riadi `viewer.html`, NIE poradie parametrov v URL.** Appenduje sa Súhrn → QES → law, ale QES sa vkladá cez `bar.insertBefore(qa, sumBtnEl)` PRED Súhrn → výsledok **QES · Súhrn · law**. Pri úprave viewera toto pravidlo zachovaj.
...
• Podpísaný originál: [názov súboru už nahratého v `docs/`, ALEBO súbor v chate]   ← POVINNÉ. Reálne ODOSLANÝ podpísaný dokument. Peter ho nahráva sám cez https://github.com/peterferenc246-design/fia/upload/main/docs — vtedy stačí názov, SHA si dohľadaj cez API. Ak príde cez chat, commitni ho INTACT. Slúži ako `doc2` aj ako cieľ `orig=`. Jazyky, ich strany aj stranu/pomer QES podpisu zisti `pypdf` (NEHÁDAJ).
...
2. **KLON SA STAVIA Z PETROVHO WORDU, nie z vlastného layoutu.** `tools/build_docx_clone.py`: naklonuje DOCX, vymení LEN text (zachová Calibri/okraje/štýly/zarovnania/tučné úseky/hypertextové e-maily/zalomenie sekcií), vloží jazykový banner ako **plávajúci ukotvený objekt** (nezaberá riadok) a čistý podpis. Prevod cez LibreOffice.
   ...
   - **VEĽKOSŤ PÍSMA: telo 12 pt** (štandard FIA FOX, platí pre klony podaní aj prekladov došlej pošty). Nadpis orgánu ~15 pt, medzihlavičky 12 pt, kontaktný blok ~9,4 pt, pätička ~8 pt. Nikdy neznižuj telo pod 12 pt kvôli zmesteniu — najprv stláčaj medzery a odstupy (fit-pass).
   - **DOŠLÁ POŠTA (cudzí dokument) sa nekloní z DOCX** — zdroj je sken s OCR vrstvou. Postup: (a) OCR text ručne skontroluj proti obrázku a oprav (typicky `0`↔`O` v spisovej značke, `S`↔`§`, rozbité dátumy), (b) preklad sádzaj nanovo do 1 strany na jazyk s jazykovým bannerom vpravo hore, (c) do pätičky KAŽDÉHO jazyka povinne **disclaimer o neoficiálnom preklade** s odkazom, že záväzný je originál dostupný cez vlajku pôvodného jazyka. Originál ide ako `doc2`, klon ako `doc1`; `qes=` sa NEPRIDÁVA (došlá pošta QES nemá).
...
5. **TRI SÚBORY — nikdy nezamieňať:**
   ...
   - **`orig=`** = jsDelivr @SHA link na ten istý `doc2` v repe (NIE OneDrive). POVINNÉ vo všetkých 19 URL.
...
7. **WIRE 19 viewer URL** na karte `case-XXXXX` — data-snap `fallback` + 9× `urls{de..sv}` + 9× viditeľných `.open` href (poradie de,en,sk,hr,pl,es,it,fr,sv):
   `viewer.html?doc=<@SHA/doc1>&doc2=<@SHA/doc2>&orig=<@SHA/doc2>&langs=CODE:PAGE[:2],...&qes=<STR:DOC:POMER>[&law=CELEX:<kod>~<popis> ak vypýtané]&vw=<N>&d=<1|2>&page=<N>`
   ...
   - **`qes=` a `vw=` patria do KAŽDEJ z 19 URL** (§1). `qes` sa NEODVODZUJE — stranu a pomer vytiahni z podpisového widgetu PDF. Po nasadení vizuálne over: tlačidlo musí trafiť podpisový blok, nie vrch strany.
   - **VLÁKNO ODPOVEDE — SPÁROVAŤ `data-thread` NA OBOCH POLOŽKÁCH.** Ak má nová položka `replyTo`, NESTAČÍ vložiť ju samu. **Partnerskú položku dohľadaj podľa predmetu z `replyTo` a v tom istom kroku jej doplň** rovnaký `data-thread="thr-<slug predmetu pôvodného podania>"` — na `<div class="item …>` aj do `data-snap.thread`. Bez toho vlákno neexistuje.
   - **ČÍSLA V KRÚŽKOCH NEPÍŠ DO MARKUPU.** Krúžok `.thr` vykresľuje `kauzy.js` (`renderThreadNumbers()`) a platí preň:
     (a) krúžok dostanú **LEN položky vo vlákne** (tie, čo majú `data-thread`); položka bez vlákna krúžok NEMÁ a ak tam ostal, JS ho odstráni;
     (b) číslo = **poradie dokumentu v rámci JEHO vlákna** chronologicky (1 = pôvodné podanie, 2 = prvá reakcia, 3 = ďalšia …), NIE poradie na karte ani číslo vlákna;
     (c) prepočítava sa automaticky pri načítaní — pri pribudnutí ďalšieho dokumentu do vlákna sa čísla posunú samy.
     Klik na krúžok alebo na riadok „Odpoveď na" zvýrazní celé vlákno.
